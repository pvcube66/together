...
async function DashboardHomeContent() {
  ...
  return (
    ...
    >
      <div className={
          'relative z-10 grid min-w-0 grid-cols-1 gap-4 overflow-x-hidden sm:gap-6 md:min-h-0 md:flex-1 md:gap-7 xl:min-h-[min(34rem,calc(100dvh-17rem))] ' +
          (settings.soloMode
            ? 'xl:grid-cols-[minmax(0,1fr)] xl:gap-8'
            : 'xl:grid-cols-[minmax(0,1.9fr)_minmax(0,3.1fr)] xl:gap-8')
        }>
        {!settings.soloMode && <Leaderboard boards={boards} currentUserId={session.user.id} />}
        <VideoPlayerWrapper />
      </div>
